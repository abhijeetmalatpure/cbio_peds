...
#entrez <- getBM(c("hgnc_symbol", "entrezgene_id"), filters = "hgnc_symbol", values = hugo, ensembl, useCache = FALSE)
#
...
sample_names <- data.table(colnames(raw_genes))
...
sample_names$valid <- ifelse(sample_names$id %in% sample_final$sample_id | sample_names$id == "Hugo_Symbol", TRUE, FALSE)

# Filter out sample columns that are not present in the data_patient_samples
genes <- rbind(transpose(sample_names[, c(3,2)]), raw_genes, use.names = FALSE)
...
sampleIds <- genes[1, 2:ncol(genes)]
