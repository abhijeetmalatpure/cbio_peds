...
# Combine data from all Foundation fnv files, filter fusion effect rows, remove chr prefix from chromosomes
...
fusion_final <- fusion_df[, c("gene1", "filename", "Fusion1", "sequence_type", "Frame")]
fusion_final <- rbind(fusion_final, setNames(fusion_df[, c("gene2", "filename", "Fusion2", "sequence_type", "Frame")], names(fusion_final)))
colnames(fusion_final) <- c("Hugo_Symbol", "Tumor_Sample_Barcode", "Fusion", "Fusion_Status", "Frame")
